...
public class DeleteBadgeCommandHandler implements Command.CommandHandler<DeleteBadgeCommand, Boolean> {
    ...
    @Override
    @Transactional
    public Boolean handle(DeleteBadgeCommand command) {
        log.info("Handling DeleteBadgeCommand: id={}", command.getId());
        ...
        propagateBadgeDeletionToBookEditions(badge);

        return true;
    }

    private void propagateBadgeDeletionToBookEditions(Badge badge) {
        // 1. Books that have this badge as parent badge
        ...
        for (Book book : booksWithBadge) {
            for (BookEdition edition : book.getEditions()) {
                if (edition.isDeleted())
                    continue;
                publishEditionSync(edition, book, null);
                evictEditionCache(edition.getId());
            }
        }

        // 2. Editions with this badge via editions_badges
        ...
        for (EditionBadge eb : editionBadges) {
            ...
            publishEditionSync(edition, book, book.getBadge());
            evictEditionCache(edition.getId());
        }

        // 3. Invalidate badge editions Redis Set
        try {
            String setKey = cacheProperties.buildKey(KeyConstants.SECTION_BADGE_EDITIONS, badge.getId().toString());
    ...
    private void publishEditionSync(BookEdition edition, Book book, Badge parentBadge) {
    ...
    private void evictEditionCache(UUID editionId) {
        try {
            cacheService
                    .remove(cacheProperties.buildKey(KeyConstants.SECTION_BOOK_EDITION_DETAIL, editionId.toString()));
        } catch (Exception ex) {
